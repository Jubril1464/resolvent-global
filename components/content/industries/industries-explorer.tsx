...
import { INDUSTRIES } from "./industries-config"

export function IndustriesExplorer() {
  const [activeId, setActiveId] = useState(INDUSTRIES[0].id)
  const tabRefs = useRef<Array<HTMLButtonElement | null>>([])

  const activeIndex = INDUSTRIES.findIndex((industry) => industry.id === activeId)
  ...
  function onTabKeyDown(event: React.KeyboardEvent<HTMLButtonElement>, index: number) {
    ...
    setActiveId(INDUSTRIES[next].id)
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] lg:gap-8">
      ...
      >
        <IndustryVisual icon={active.icon} />
